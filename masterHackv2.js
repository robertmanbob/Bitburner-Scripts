...
export function calculateGrowthThreadsWithFormula(ns, server, player, start = 0, goal = server.moneyMax, maxIterations = 100) {
	...
	if (start >= goal) {    // We're already there!
		return 0;
	}
	...
	for (let i = 0; guess != best && i < maxIterations; ++i) {
		...
		if (expectedOutcome >= goal) {  // This many threads will reach our goal!
			if (best < guess) { // This shouldn't be possible, but better safe than sorry.
				return best;
			}
...
export async function main(ns) {
	...
	function calcRatio(tgt, plyr, percent = 0.1) {
		...
		//ns.print(ns.hackAnalyzeSecurity(ratio.hackthreads) + " " + ratio.hackthreads)
		//ratio.growthreads = calculateGrowthThreadsWithFormula(ns, tgt, plyr, tgt.moneyMax * (1 - percent))
	...
	function findGreatestRam(target, player, farm) {
		...
		while (!found) {
			...
			let total = (Math.max(ratio.weakenGrow, ratio.weakenHack) + Math.max(ratio.hackthreads, ratio.growthreads)) * 1.75
	...
	async function prepCycle(tgt, frm) {
		let maxThreads = (ns.getServerMaxRam(farm) - ns.getServerUsedRam(farm)) / 1.75
	...
	//ns.print("TEST TEST TEST" + ratioPercent)
	...
	while (true) {
		...
		await ns.sleep(ns.getWeakenTime(target.hostname) - ns.getHackTime(target.hostname) - 200)
		ns.exec("slaveHack.ns", farm, ratio.hackthreads, target.hostname)
		await ns.sleep(ns.getHackTime(target.hostname) + 300)
		ratio = calcRatio(target, player, ratioPercent)
		ns.exec("slaveWeaken2.ns", farm, ratio.weakenGrow, target.hostname)
		await ns.sleep(ns.getWeakenTime(target.hostname) - ns.getGrowTime(target.hostname) - 200)
		ns.exec("slaveGrow.ns", farm, ratio.growthreads, target.hostname)
		await ns.sleep(ns.getGrowTime(target.hostname) + 1000)
